...
export const options = buildSharedIterationsOptions(6, 20, '5m');
...
const params = scenarioParams({
  ...
  pollTimeoutMs: parseInt(__ENV.POLL_TIMEOUT_MS || '90000', 10),
});

const patterns = [
  {
    prompt: () => `reliability-success-${randomSuffix()}`,
    model: 'gpt-sim-a',
    requiredMemoryMb: 2048,
    weight: 100,
  },
  {
    prompt: () => `phase7-fail-retry-once-${randomSuffix()}`,
    model: 'gpt-sim-b',
    requiredMemoryMb: 4096,
    weight: 200,
  },
  {
    prompt: () => `phase7-slow-timeout-${randomSuffix()}`,
    model: 'gpt-sim-mix',
    requiredMemoryMb: 4096,
    weight: 300,
  },
  {
    prompt: () => `phase7-fail-always-${randomSuffix()}`,
    model: 'gpt-sim-b',
    requiredMemoryMb: 4096,
    weight: 250,
  },
...
export default function () {
  const pattern = patterns[__ITER % patterns.length];
  const payload = {
    prompt: pattern.prompt(),
    model: pattern.model,
    requiredMemoryMb: pattern.requiredMemoryMb,
...
export function handleSummary(data) {
  return { stdout: formatSummary('Reliability Mix Scenario', data) };
}
